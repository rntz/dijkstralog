...
fn main() {
    ...
    // trans b a <- edge a b
    // trans c a <- trans b a, edge b c
    //
    // implemented as
    //
    // Δtrans 0     b a = edge a b
    // Δtrans (i+1) c a = Δtrans i b a * edge b c
    //  trans (i+1)     = trans i + Δtrans i

    let mut trans: LSM<Key<(u32, u32)>> = LSM::new();
    let mut rev_edges: Vec<Key<(u32, u32)>> = edges.iter().map(|&(a, b)| (b, a).into()).collect();
    rev_edges.sort_by_key(|x| x.key);
    let mut delta_trans: Layer<Key<(u32, u32)>> = Layer::from_sorted(rev_edges);
    ...
    while !delta_trans.as_slice().is_empty() {
        ...
        // -- COMPUTE DELTA:  Δtrans' c a = Δtrans b a * edge b c
        print!("Delta rules ");
        std::io::stdout().flush().unwrap();
        let mut new_paths: Vec<(u32, u32)> = Vec::new();
        dijkstralog::nest! {
            for (b, (delta_trans_b, edges_b)) in
                ranges(delta_trans.as_slice(), |x| x.key.0)
                .join(ranges(edges, |x| x.0))
                .iter();
            for &(_, c) in edges_b;
            for &Pair { key: (_, a), value: _ } in delta_trans_b;
            new_paths.push((c, a))
        }
