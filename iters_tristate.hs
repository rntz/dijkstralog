...
data Value v = NotYet | No | Yes v deriving (Show, Eq, Functor)
...
data Position k v = Done | At !k (Value v) deriving (Show, Eq, Functor)
...
toSorted (Iter (At k vopt) seek) =
  case vopt of
    Yes v  -> (k, v) : toSorted (seek Greater k)
    No     -> toSorted (seek Greater k) --advance
    NotYet -> toSorted (seek AtLeast k) --keep computing

fromSorted :: Ord k => [(k,v)] -> Iter k v
fromSorted l = Iter p s where
  p | (k,v):_ <- l = At k (Yes v)
    | otherwise    = Done
  s b tgt = fromSorted $ dropWhile (not . matches b tgt . fst) l

traceFromSorted name l = Iter p s where
  p | (k,v):_ <- l = At (trace (name ++ " " ++ show k) k) (Yes v)
    | otherwise    = Done
