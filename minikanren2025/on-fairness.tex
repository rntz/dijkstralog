...
We can compute this using \ttt{intersect}, but performance is \emph{dramatically} better if we avoid intersecting \ttt{evens} and \ttt{odds} directly by intersecting one of them with \ttt{ends} first.
At the GHCi repl:
...
\footnotetext{
  We are telling a white lie here: because we use Haskell lists, seeking is linear-time, and there is no asymptotic slow-down; we are instead observing the difference between an \emph{interpreted} inner loop (\ttt{intersect}, loaded at the GHCi repl) and a \emph{compiled} one (\ttt{dropWhile} from the standard library).
