...
\noindent
In principle, the union of streams should be productive if \emph{any} input stream is.
...
\todo{
  fundamental idea: an iterator doesn't have to be either empty or yield a concrete key-value pair.
  it might still be in the process of finding the next key-value pair.
  but even in this case, it does know a \emph{lower bound} on the next key.
  the fact that it can stop in the middle lets us avoid being forced to do arbitrary amounts of work (bounded); and the lower bound on key-space is what we need to propagate to the next iterator we choose to schedule.
}
