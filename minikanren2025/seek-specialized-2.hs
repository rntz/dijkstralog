...
data Position k v = Found k v | Bound (Bound k) | Done deriving (Eq, Show)

instance Ord k => Ord (Bound k) where
  compare x y = embed x `compare` embed y
    where embed (Atleast x) = (x, 0)
          embed (Greater x) = (x, 1)
...
bound it = case posn it of Bound p -> p; Found k _ -> Atleast k; Done -> undefined

instance (Seek s, Seek t, Key s ~ Key t) => Seek (s, t) where
  type Key   (s, t) = Key s
  type Value (s, t) = (Value s, Value t)
  posn (s, t) | Done <- posn s = Done --important this is checked first
              | Done <- posn t = Done
              | Found k x <- posn s, Found k' y <- posn t, k == k' = Found k (x, y)
              | otherwise = Bound (bound s `max` bound t)
  seek (s, t) p = (s', t') where --important that this is lazy in 2nd arg
    s' = seek s p
