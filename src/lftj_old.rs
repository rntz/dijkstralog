...
pub trait Seek: Iterator {
    ...
    // TODO: is this convenience method worth it?
...

// ---------- TRIE ITERATORS ----------
pub trait TrieSeek: Seek {
    fn level(&self) -> isize;   // initially -1, range [-1,height())
    fn height(&self) -> usize;
    fn enter(&mut self);        // increases level. precondition: level+1 < height
    fn leave(&mut self);        // decreases level. precondition: -1 < level
}

pub struct TrieJoin<Iter> {
    height: usize,
    level: isize,
    // uh oh. bumping up against rust's ownership discipline here. Talk to Max.
    iters: Vec<Vec<Iter>>,
    frogs: Vec<Leapfrog<Iter>>,
}
